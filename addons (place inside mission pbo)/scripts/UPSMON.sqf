...
// Vars that seem to not be declared before being used?
_lastcurrpos = getPosATL (_this select 0);
...
// TBD: add to global side arrays?
	...
	// create the clones
		for "_grpcnt" from 1 to _copies do {
			...
			nul=[_lead,_areamarker,_pause,_noslow,_nomove,_nofollow,_initpos,_track,_showmarker,_shareinfo,"DELETE:",_dead] execVM "addons\scripts\upsmon.sqf";
			//sleep .05;
		};
		processInitCommands;
		sleep .05;
...
// **********************************************************************************************************************
		...
		if (_pursue) then {
			...
			{
				if ((_i != _grpid) && (format (["%1", _x]) != "[0,0]") && !isNull(_x)) then {

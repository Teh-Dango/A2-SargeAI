...
//Adding eventhandlers
	"KRON_UPS_EAST_SURRENDED" addPublicVariableEventHandler { if (_this select 1) then { nul=[east] execvm "addons\scripts\UPSMON\MON_surrended.sqf";};};
	"KRON_UPS_WEST_SURRENDED" addPublicVariableEventHandler { if (_this select 1) then { nul=[west] execvm "addons\scripts\UPSMON\MON_surrended.sqf";};};
	"KRON_UPS_GUER_SURRENDED" addPublicVariableEventHandler { if (_this select 1) then { nul=[resistance] execvm "addons\scripts\UPSMON\MON_surrended.sqf";};};
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	if (isNil("KRON_UPS_INIT") || KRON_UPS_INIT == 0) then {
		...
		call compile preprocessFileLineNumbers "addons\scripts\UPSMON\common\MON_functions.sqf";

		//init !R functions
		call compile preprocessFileLineNumbers "addons\scripts\UPSMON\!R\R_functions.sqf";

		if (isNil "RE") then {[] execVM "\ca\Modules\MP\data\scripts\MPframework.sqf"};

		//scripts initialization
		UPSMON = compile preprocessFile "addons\scripts\UPSMON.sqf";
		UPSMON_surrended = compile preprocessFile "addons\scripts\UPSMON\MON_surrended.sqf";
	...
	// ***********************************************************************************************************
		MON_MAIN_server = {
			...
			private["_obj","_trg","_l","_pos","_countWestSur","_countEastSur","_countResSur","_WestSur","_EastSur","_ResSur","_target","_targets","_targets0","_targets1","_targets2","_npc","_cycle","_arti","_side","_range","_rounds","_area","_maxcadence","_mincadence","_bullet","_fire","_knownpos","_sharedenemy","_enemyside","_timeout"];
			...
			while {true} do
			{
				...
				{
					if (!(isNil _x) && !(isNull _x) && (alive _x) && !(captive _x)) then {
						_npc = _x;
						_targets = [];
